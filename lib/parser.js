...
var envs = require('./env.js')
  ...
  , Symbol = _.Symbol
  , str = _.str
  , Callable = require('./callable.js').Callable
  , Counter = require('./callable.js').Counter
...
var evaluate = function e(x, env, counter) {
  ...
  var fns = {
    ...
    , 'define': function(x) {
        var v = ce(x[1]);
        env.set(x[0].val, v);
        return _.isa(v, Callable) ? '[Function]' : str(v);
      }
    , 'lambda': function(x) {
        ...
        var f = function recur() {
          var args = _.destructure(x[0].map(vals), _.args(arguments))
            , lenv = envs.init([], [], env).set('recur', new Callable(recur)).update(args);
  ...
  return Callable.prototype.call.apply(f, exps);
};
