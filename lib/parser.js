...
var evaluate = function e(x, env) {
  ...
  var fns = {
    ...
    , 'gensym': function(x) {
        return new Symbol('GS#' + ce((x[0] || 'symbol')) + Math.floor(Math.random()*10000));
      }
    , 'unquote': function(x) {
  ...
  var f = ce(x[0])
    , isMacro = function(value) {
        return _.isArray(value) && _.findKeyInPairs(f, 'macro');
      };

  if (isMacro(f)) {
    // Macroexpansion
    return ce(ce([new Symbol('macroexpand1'), [new Symbol('quote'), x]]));
