// the parser

var envs = require('./env.js')
  , _ = require('./utils.js')
  , Symbol = _.Symbol
  , str = _.str;

var tokenize = function(str) {

  var state = []
    , tokens = []
    , current = []
    , escape = false
    , isDelim = function(s) {
        return s === '"';
      }
    , currState = function() {
        return (!!state.length) ? _.last(state) : null;
      }
    , isCurrStateDelim = function(c) {
        return isDelim(c) && currState() === c;
      }
    , addToken = function() {
        tokens.push(current.join(""));
        current = [];
      }
    , iter = function(ch) {

        var c = currState();

        if (!escape && !c) {
          if (['(', ')'].indexOf(ch) >= 0) {
            addToken();
            current = [ch];
            ch = ' ';
          }
          if (ch == ' ') {
            addToken();
            return;
          }
        }

        current.push(ch);

        if (escape) {
          escape = false;
        } else if (ch === '\\') {
          escape = true;
        } else if (!c && isDelim(ch)) {
          state.push(ch);
        } else if (isCurrStateDelim(ch)) {
          state.pop();
        }
      }
    , hasLength = function(i) {
        return i.length > 0;
      };

  str.split("").forEach(iter);
  addToken();

  return tokens.map(_.trim).filter(hasLength);
};

var quote = function(x) {
  return x[0];
};

var evaluate = function e(x, env) {

  var ce = function(x) {
    return e(x, env);
  };

  var fns = {
    'quote': quote,
    'q': quote,
    'atom?': function(x) {
      var evald = ce(x[0]);
      return !_.isArray(evald);
    },
    'eq?': function(x) {
      var v1 = ce(x[0]), v2 = ce(x[1]),
        val = function(v) {
          var r = JSON.stringify(v || []);
          return r;
        };
      return val(v1) == val(v2);
    },
    'car': function(x) {
      return (ce(x[0]) || [])[0];
    },
    'cdr': function(x) {
      return _.rest(x.map(ce)[0]);
    },
    'cons': function(x) {
      return [ce(x[0])].concat(ce(x[1]));
    },
    'cond': function(x) {
      return x.reduce(function(a, b) {
        return (a) ? a : ((ce(b[0])) ? ce(b[1]) : false)
      }, false);
    },
    'null?': function(x) {
      var e = ce(x[0]);
      return typeof e == 'undefined' || e === null || (_.isArray(e) && !e.length);
    },
    'if': function(x) {
      var test = x[0], cons = x[1], alt = x[2];
      return ce((ce(test) ? cons : alt));
    },
    'when': function(x) {
      var test = x[0], cons = x[1];
      return ce(test) ? ce(cons) : null;
    },
    'set!': function(x) {
      var v = ce(x[1]);
      env.find(x[0].val)[x[0].val] = v;
      return v;
    },
    'define': function(x) {
      var v = ce(x[1]);
      env.set(x[0].val, v);
      return str(v);
    },
    'lambda': function(x) {
      var vals = function(a) {
        return a.val;
      };

      return function() {
        return e(x[1], envs.init(x[0].map(vals), _.args(arguments), env));
      };
    },
    'begin': function(x) {
      return x.reduce(function(a, b) { return ce(b); }, null);
    },
    'str': function(x) {
      var checkStr = function(s) {
        return (s.split && _.isString(s))
          ? s.split('').slice(1, s.length - 1).join('')
          : s;
      };

      return '"' + x.map(ce).map(checkStr).map(str).join('') + '"';
    }
  };

  if (!env['*globals*'] && !env.outer) {
    env.update(fns);
    env['*globals*'] = true;
  }

  if (_.isa(x, Symbol)) {
    return env.find(x.val)[x.val];
  }

  if (!_.isArray(x)) {
    return x;
  }

  if (x[0].val in fns) {
    return fns[x[0].val](_.rest(x), x[0].val);
  }

  var exps = x.map(ce), f = exps.shift();

  return f.apply(null, exps);
};


var reader = function r(tokens) {

  if (!tokens.length) {
    throw "Unexpected OEF";
  }

  var token = tokens.shift();
  if (token == '(') {
    var L = [];
    while (tokens[0] != ')') {
      L.push(r(tokens).found);
    }
    tokens.shift();
    return { found: L, rest: tokens };
  }

  if (token == ')') {
    throw "Unexpected )";
  }

  return { found: _.atom(token), rest: tokens };
};

var read = function(tokens) {
  return reader(tokens).found;
};

var Parser = function() {
  this.env = envs.initGlobal();
};

Parser.prototype = {
    tokenize: tokenize
  , reader: reader
  , e: function(x) {
      return str(evaluate(x, this.env));
    }
  , parse: function(s) {
      return this.e(read(tokenize(s)));
    }
  , getEnv: function() {
      return this.env;
    }
};

exports.Parser = Parser;
