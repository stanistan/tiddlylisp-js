...
var evaluate = function e(x, env) {
  ...
  if (_.isArray(f) && _.findKeyInPairs(f, 'macro')) {
    ...
    f = env.getProp(x[0], 'macro');
    exps = _.rest(x);

    var Lambda = f.apply(null, exps);

    Lambda.body = Lambda.body.map(function r(p) {
      if (_.isArray(p)) {
        if (_.isa(p[0], _.Symbol) && p[0].val == 'macroeval') {
          return e(_.rest(p)[0], Lambda.env);
        } else {
          return p.map(r);
        }
      }
    ...
    return ce(handleResponse(Lambda));
  } else {
